...
( function( $ ) {
	...
	var PostsClassicFrontEndHandler = function( $scope, $ ) {
		...
		if ( is_infinite && 'yes' === elementSettings.classic_infinite_scroll_button ) {
			...
			infiniteScrollArgs.button = '.ee-load-button__trigger--' + $scope_id;
	...
	var ParallaxBackgroundFrontEndHandler = function( $scope, $ ) {
		...
		if ( "" !== elementSettings.parallax_background_speed.size ) {
			parallaxBackgroundArgs.parallaxSpeed = elementSettings.parallax_background_speed.size;
		}

		if ( "" !== elementSettings.parallax_background_speed_tablet.size ) {
			parallaxBackgroundArgs.parallaxSpeedTablet = elementSettings.parallax_background_speed_tablet.size;
		}

		if ( "" !== elementSettings.parallax_background_speed_mobile.size ) {
			parallaxBackgroundArgs.parallaxSpeedMobile = elementSettings.parallax_background_speed_mobile.size;
		}
	...
	var CircleProgressFrontEndHandler = function( $scope, $ ) {
		...
		var elementSettings = getElementSettings( $scope ),
			...
			$value 			= $circle.find( '.elementor-circle-progress-value .value' ),
			_value 			= parseValue( $scope.find( '.elementor-circle-progress-value' ).data('cp-value'), 75 ),
			_move_decimal 	= 0,
			_absolute 		= _value,
			_max_value 		= 100,
			_decimals 		= countDecimals( _value ),
		...
		$circle.circleProgress( cpArgs ).on( 'circle-animation-progress', function( event, progress, stepValue ) {
			...
			if ( _move_decimal ) {
				_stepValue = moveDecimal( _stepValue, _move_decimal );
			}
	...
	var parseValue = function ( value, _default ) {
		var _value = value;
